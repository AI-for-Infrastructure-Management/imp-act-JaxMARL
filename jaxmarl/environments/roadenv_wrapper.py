...
class RoadEnvironment_Wrapper(object):
    ...
    def get_obs(self, state: State) -> chex.Array:
        ...
        obs = jnp.concatenate([state.belief, _timestep, _budget, self.agent_encodings], axis=1).astype(jnp.float32)
        agent_obs = {agent: obs[i] for i, agent in enumerate(self.agents)}
