...
class EncoderBlock(nn.Module):
    ...
    def __call__(self, x, mask=None, deterministic=True):
        ...
        attended = self.self_attn(inputs_q=x, inputs_kv=x, mask=mask, deterministic=deterministic).astype(jnp.float32)
...
class ScannedTransformer(nn.Module):
    ...
    def initialize_carry(hidden_size, *batch_size):
        return jnp.zeros((*batch_size, hidden_size)).astype(jnp.float32)
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        if 'smax' in env.name.lower(): # smax agent 
            ...
            init_x = (
                jnp.zeros((1, 1, 1, sample_traj.obs[env.agents[0]].shape[-1]), dtype=jnp.float32), # (time_step, batch_size, n_entities, obs_size)
        ...
        init_x = (
            jnp.zeros((len(env.agents), 1, 1), dtype=jnp.float32), # q_vals: n_agents, time_steps, batch_size
            ...
            jnp.zeros((1, 1, 1, state_size), dtype=jnp.float32), # states: time_step, batch_size, n_entities, state_size
            jnp.zeros((1, 1), dtype=jnp.float32), # done: (time_step, batch size)
            False, # train
        )
