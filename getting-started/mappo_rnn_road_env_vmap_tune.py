...
def tune(default_config):
    ...
    #! Limitation:
    NUM_STEPS, NUM_EPOCHS, GRU_HIDDEN_DIM, FC_DIM_SIZE, cannot be
    parallelized the same way as learning rate or ent coeff, since they
    serve as static inputs.

    - NUM_STEPS, NUM_EPOCHS are inputs to lax.scan. This control loop
    only supports static arguments. For dynamic number of iters, we must
    switch to fori_loop or while_loop. In theory, this can be done, but
    skipping this in the interest of time.

    - GRU_HIDDEN_DIM, FC_DIM_SIZE must be static (Python ints) to create
    neural nets (at least Flax does not natively support dynamic shapes)
    """
    ...
    #! and Nones and 0s in the vmaps to match the inputs
    ...
    num_combinations, num_seeds, num_evals = test_returns.shape

    # Aggregate over seeds
    # shape: (num_combinations, num_evals)
    test_returns_mean = jnp.mean(test_returns, axis=1)
    test_returns_std = jnp.std(test_returns, axis=1)

    # get the hyperparameter(s) (with max. mean returns)
    ...
    print(f"Best combination mean: {test_returns_max[idx_best_comb].mean():.2f}")
    ...
    for i, hyp in enumerate(hypers_grid):
        ...
        print(f"{i}: {_label} | max: {test_returns_max[i]:.2f}")
